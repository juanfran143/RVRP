    ...
    def simulation(self, solution, capacity = 9999999999):
        ...
        for _ in range(self.simulations):
            ...
            for route in solution:
                ...
                for edge in route.route[:-1]:
                    aux = np.random.lognormal(np.log(edge.y.demand), self.var_demand)
                    demand += aux
                    distance += np.random.lognormal(np.log(edge.dist), self.var_distance)
                    if demand > capacity:
                        demand = aux
                        distance += (edge(edge.x, route.route[0].x)).dist + (edge(route.route[0].x, edge.y)).dist - edge.dist
                        if first:
                            fail += 1
